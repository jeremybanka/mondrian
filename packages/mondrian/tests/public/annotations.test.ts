...
import type { PdfRenderOptions, RenderedPdf } from "mondrian.pdf/testing"

it("renders annotation appearances only when requested", async () => {
	...
	for (const enabled of [true, false]) {
		const options: PdfRenderOptions = {
			resolution: 72,
			renderAnnotations: enabled,
		}
		const rendered: RenderedPdf = await renderPdf(bytes, options)
		const page = rendered.pages[0]!
		const offset = (19 * page.width + 20) * 4
		expect(Array.from(page.pixels.slice(offset, offset + 4))).toEqual(
			enabled ? [255, 0, 0, 255] : [255, 255, 255, 255],
		)
	}
})
